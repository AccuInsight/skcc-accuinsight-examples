...
def network(X_train, y_train, X_test, y_test):
    ...
    conv1_1 = Convolution1D(config.num_node, (6), activation='relu', input_shape=im_shape)(inputs_cnn)
    conv1_1 = BatchNormalization()(conv1_1)
    pool1 = MaxPool1D(pool_size=(3), strides=(2), padding="same")(conv1_1)
    conv2_1 = Convolution1D(config.num_node, (3), activation='relu', input_shape=im_shape)(pool1)
    conv2_1 = BatchNormalization()(conv2_1)
    pool2 = MaxPool1D(pool_size=(2), strides=(2), padding="same")(conv2_1)
    conv3_1 = Convolution1D(config.num_node, (3), activation='relu', input_shape=im_shape)(pool2)
    conv3_1 = BatchNormalization()(conv3_1)
    pool3 = MaxPool1D(pool_size=(2), strides=(2), padding="same")(conv3_1)
    flatten = Flatten()(pool3)
    dropout1 = Dropout(config.dropout_rate)(flatten)
    dense_end1 = Dense(config.num_node, activation='relu')(dropout1)
    dropout2 = Dropout(config.dropout_rate)(dense_end1)
    dense_end2 = Dense(config.num_node/2, activation='relu')(dropout2)
